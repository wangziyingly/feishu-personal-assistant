...
PDF_MAX_CHARS = 30000       # 送入大模型的正文字符上限
...
REPORT_PROMPT = """你是一位在顶级会议发表过多篇论文的研究者，正在为组会准备文献汇报。
基于给出的论文内容（可能有截断），输出一份中文文献汇报，要求深入、批判、有洞察，不要泛泛而谈。
严格按以下结构输出（保留小节标题）：

【一句话总结】
【研究背景与动机】为什么这个问题值得做
【核心问题与挑战】
【方法详解】关键设计、架构/公式用通俗语言解释、为什么这样设计
【实验与结果】数据集、基线、关键数字
【创新点与贡献】
【局限与批判性思考】至少 2 条，审稿人视角
【对后续工作的启发】可以接着做的 1-2 个方向
...
class PaperModule:
    ...
    def _report_from_abstract(self, user_id, paper):
        ...
        report = self.llm.chat([
            {"role": "system", "content":
                "你是论文解读助手。只有论文标题和摘要可用，请基于摘要输出文献汇报，"
                "结构参照：【一句话总结】【研究背景与动机】【方法（据摘要推断）】【主要结论】"
                "【局限与批判性思考】【对后续工作的启发】。"
                "摘要未覆盖的部分明确写「摘要未提及」，不要编造。"},
